...
class UnconditionalLDMPipeline(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        latents_shape = (num_sampling, self.unet.config.in_channels, height // 8, width // 8)

        if latents is None:
            latents = randn_tensor(
                latents_shape, generator=generator, device=self._execution_device
            )
        else:
...
class ConditionalLDMPipeline(DiffusionPipeline):
    r"""
    Pipeline for text-to-image generation using latent diffusion.
