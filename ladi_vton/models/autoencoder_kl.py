...
class AutoencoderKL(ModelMixin, ConfigMixin):
    ...
    def _decode(self, z: torch.FloatTensor, inetermediate_features: list = None, int_layers: list = None, 
                return_dict: bool = True) -> Union[DecoderOutput, torch.FloatTensor]:
        if self.use_tiling and (z.shape[-1] > self.tile_latent_min_size or z.shape[-2] > self.tile_latent_min_size):
            return self.tiled_decode(z, return_dict=return_dict)

        z = self.post_quant_conv(z)
        if inetermediate_features:
            dec = self.decoder(z, inetermediate_features, int_layers)
        else:
            dec = self.decoder(z)
    ...
    @apply_forward_hook
    def decode(self, z: torch.FloatTensor, inetermediate_features: list = None, int_layers: list = None, 
               return_dict: bool = True) -> Union[DecoderOutput, torch.FloatTensor]:
        if self.use_slicing and z.shape[0] > 1:
            ...
            if inetermediate_features:
                decoded = self._decode(z, inetermediate_features, int_layers).sample
            else:
    ...
    def tiled_encode(self, x: torch.FloatTensor, return_dict: bool = True) -> AutoencoderKLOutput:
        ...
        for i in range(0, x.shape[2], overlap_size):
            ...
            for j in range(0, x.shape[3], overlap_size):
                ...
                tile, inetermediate_features = self.encoder(tile)
        ...
        return AutoencoderKLOutput(latent_dist=posterior), inetermediate_features
